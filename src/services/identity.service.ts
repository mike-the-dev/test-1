...
export class IdentityService {
  ...
  /**
   * Creates a new chat session METADATA record without writing an IDENTITY record.
   * Used by email-inbound Case 2 and Case 3-stale, where routing is driven by the
   * recipient's local-part and the Customer GSI — not IDENTITY records.
   *
   * Writing IDENTITY records for email-inbound sessions would cause re-use problems:
   * a returning customer's stale email would find the old IDENTITY and route there
   * instead of creating a fresh session as the dispatcher intends.
   */
  async createSessionWithoutIdentity(source: string, accountUlid?: string): Promise<string> {
    const table = this.databaseConfig.conversationsTable;
    const sessionUlid = ulid();
    const now = new Date().toISOString();
    const sessionPk = `${CHAT_SESSION_PK_PREFIX}${sessionUlid}`;

    this.logger.log(`Creating email-inbound session (no identity record) [sessionUlid=${sessionUlid} source=${source}]`);
