...
- Phase 2 IDENTITY removal + style-pass (IdentityService→SessionService rename; lookupOrCreateSession deleted; createSessionWithoutIdentity→createSession; sessionId in wire contract; CreateSessionResult→ChatSessionCreateSessionResult; updateOnboarding inline type→ChatSessionUpdateOnboardingResult; as const→satisfies in web-chat.controller.spec.ts): 587 tests passing (36 suites), 0 failures, build clean. Time: ~2.565 s.
- Phase 3 verification-code tool guard + customerFound→isReturningVisitor rename + lead_capture/shopping_assistant prompt tightening + style-pass (11 `expect("customerFound" in parsed).toBe(false)` → `expect(parsed.customerFound).toBeUndefined()`, multi-line block comment trimmed): 590 tests passing (36 suites), 0 failures, build clean. Time: ~1.69 s.
- SessionService restructure (createSession→lookupOrCreateSession, 4-arg signature, lookup-or-mint policy moved from WebChatController into service; METADATA writes agent_name; pointer record agent_name parameterized) + style-pass (`: unknown` on catch clause; Crockford-invalid chars fixed in 5 fixture constants in session.service.spec.ts and email-reply.service.spec.ts): 590 tests passing (36 suites), 0 failures, build clean. Time: ~1.879 s.
- null-init removal fix + style-pass (removed `customer_id`, `continuation_from_session_id`, `continuation_loaded_at` null-init setClauses from SessionService.lookupOrCreateSession Branch B; made those 3 fields optional on the type; updated 1 existing test + added 1 new regression test in collect-contact-info.tool.spec.ts describe block 15; removed `as Record<string, string>` cast in new test replaced with optional chaining): 591 tests passing (36 suites), 0 failures, build clean. Time: ~2.901 s.
- 7-field ID-prefix consistency change + style-pass (DDB attribute values prefixed CHAT_SESSION#/A#/C#/G#; sessionUlid→sessionId rename on EMAIL_INBOUND; defensive normalization at 4 read sites; nested ternaries replaced with let+if blocks at 2 sites; 3 `as Record<string, string>` casts removed from preview-cart.tool.spec.ts with optional chaining): 597 tests passing (36 suites), 0 failures, build clean. Time: ~2.078 s.
- check_active_cart feature close-out + style-pass (check-active-cart.tool.spec.ts added with 7 tests; shopping-assistant.agent.spec.ts still 12 tests): 606 tests passing (37 suites), 0 failures, build clean. Time: ~1.976 s.
- Phase A prompt-restructure + spec split (shopping-assistant.agent.ts "acknowledge warmly" folded into has_cart:false branch only; one assertion split into two `it` blocks in shopping-assistant.agent.spec.ts): 607 tests passing (37 suites), 0 failures, build clean. Time: ~1.903 s.
